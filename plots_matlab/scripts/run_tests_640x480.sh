...
do
	...
	do
		...
		START_TIME=$SECONDS
		./campower -c -t $TIME -n $SAMP -iw $WAIT -dusb -o dat/SEN_calib_${w}x${h}_r${r}_usboff.dat > tmp.tmp
		...
		./campower -c -t $TIME -n $SAMP -iw $WAIT -o dat/SEN_calib_${w}x${h}_r${r}_usbon.dat > tmp.tmp
		echo "$(($(($SECONDS - $START_TIME))/60)) min $(($(($SECONDS - $START_TIME))%60)) sec"  
		...
		do
			...
			START_TIME=$SECONDS
			./campower -s -fps $fps -w $w -h $h -t $TIME -n $SAMP -iw $WAIT -i $id -o dat/SEN_act_${w}x${h}\@${fps}_r${r}.dat > tmp.tmp
			echo "$(($(($SECONDS - $START_TIME))/60)) min $(($(($SECONDS - $START_TIME))%60)) sec"    

			echo -n "#$r-$s running idle   test ${w}x${h} at ${fps} fps..."
			START_TIME=$SECONDS
			./campower -s -fps $fps -w $w -h $h -t $TIME -n $SAMP -iw $WAIT -i $id -nc -o dat/SEN_idle_${w}x${h}\@${fps}_r${r}_nc.dat > tmp.tmp
			echo "$(($(($SECONDS - $START_TIME))/60)) min $(($(($SECONDS - $START_TIME))%60)) sec"  
		done
