function [res]=load_wifi_data(input_dir,label,disp)
...
tline = fgets(fID); MaxBitrate       = sscanf(tline,'MaxBitrate=%d');
tline = fgets(fID); nPackets       = sscanf(tline,'nPackets=%d');
tline = fgets(fID); pktSize       = sscanf(tline,'pktSize=%d');
...
res = struct ('file',filename,'runID',runID,'name',name,...    
    'iniTX',iniTX,'endTX',endTX,'iniRX',iniRX,'endRX',endRX,...
    'difTX',endTX-iniTX,'difRX',endRX-iniRX,...
    'MaxBitrate',MaxBitrate,'nPackets',nPackets,'pktSize',pktSize,...
