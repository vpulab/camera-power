function [res]=load_cam_data(input_dir,label,disp)
...
for f=1:numel(res)
    ...
    if numel(res{f}.captureT)>1
        res{f}.captureT = res{f}.captureT(1:minSamples);    
        res{f}.sleepT = res{f}.sleepT(1:minSamples);    
    end
end
...
tline = fgets(fID); fps       = sscanf(tline,'fps=%f');
...
res = struct ('file',filename,'runID',runID,'name',name,...
    'width',width,'height',height,'fps',fps,'realfps',realfps,...
    'reads',reads,'time',time,'samples',samples,'startTime',startTime,'endTime',endTime,...
    'energy',energy,'power',power,'voltage',voltage,'captureT',captureT,'sleepT',sleepT);
