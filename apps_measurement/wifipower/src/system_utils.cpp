...
int disableUSBdevice(sim_config_t config, std::string usbdevice, std::string usbport)
{
	...
	//system("echo your_pass_here | sudo -S ifconfig ra0 down"); //disable interface
...
int enableTestConditions(sim_config_t config)
{
	...
	std::cout << "Setting frequency of CPU0 to 1.20 GHz" << std::endl;
	if(!system_admin(config,"sudo -S cpufreq-set -g userspace"))	return -1;//enable CPU governor specified by user
	if(!system_admin(config,"sudo -S cpufreq-set -f 1000MHz")) return -1;
	...
	//if(system(!system_admin(config,"sudo -S stop network-manager")) return-1; //enable network manager
...
int disableTestConditions(sim_config_t config)
{
	...
	if(!system_admin(config,"sudo -S ifconfig ra0 up"))	return -1;//enable eth0
...
int parseCmdOptions(int argc, char *argv[], sim_config_t& config)
{
	...
	config.dataPkt = 1000; //default size of exchanged packets
	...
	if(cmdOptionExists(argv, argv+argc, "-d"))
		config.dataPkt = atoi(getCmdOption(argv, argv + argc, "-d"));

	if(cmdOptionExists(argv, argv+argc, "-max"))
		config.maxBitrate = atof(getCmdOption(argv, argv + argc, "-max"));

	if(cmdOptionExists(argv, argv+argc, "-n"))
		config.samples = atof(getCmdOption(argv, argv + argc, "-n"));

	if(cmdOptionExists(argv, argv+argc, "-i"))
		config.interface=getCmdOption(argv, argv + argc, "-i");
...
int list_all_usb_devices() {
	...
	std::cout<<cnt<<" USB Devices in list."<<std::endl; //print total number of usb devices
...
std::string getFullPortUSB(std::string id) {
	...
	for(ssize_t i = 0; i < cnt; i++)
	{
		...
		if (tmpid.compare(id)==0)
		{
			...
			std::cout << " usb device " << tmpid << " (wifi adapter) found with port=" << port<< std::endl;
		}
	}
