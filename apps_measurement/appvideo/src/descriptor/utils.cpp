...
void print_huffman_tree(Node *node, unsigned int depth) {
    if (node == NULL)
        return;
...
Node *create_huffman_tree(std::vector<Node>& leaves) {
    // cola de punteros a nodos, ordenada por frecuencia
    typedef std::priority_queue<Node *, std::vector<Node *>, DereferenceLess> NodePtrQueue;
    NodePtrQueue nodes;
...
void update_huffman_code(HuffmanCode& code, Node *node, const std::string& prefix) {
    if (node == NULL)
        return;
    if (node->is_leaf())
...
int encode_huffman(std::stringstream& text_stream, std::stringstream& salida) {
    ...
    copy(encoded_msg.begin(),encoded_msg.end(),std::ostream_iterator<unsigned char>(salida));
