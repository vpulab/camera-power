...
int enableTestConditions(sim_config_t config)
{
	...
	if(system("cpufreq-info | grep -i \"Analyzing CPU\"")) return -1;

	if(system("rfkill block bluetooth")) return -1;//disable bluetooh
	//if(!std::system("echo your_pass_here | sudo -S stop network-manager")) return-1; //disable network manager
